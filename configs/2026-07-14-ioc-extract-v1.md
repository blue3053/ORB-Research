...
- `scope`는 ip, domain, url, cert, spki, ssh_key, jarm, ja3, ja4, port_protocol, http_device_hint, hash_md5, hash_sha1, hash_sha256, email, mutex, filepath 중 하나다.
- `context`는 malicious, victim, legitimate_infra, relay_node, controller, staging, c2, scanner, sinkhole, unknown 중 하나다.
- 침해된 SOHO router, botnet node, ORB relay처럼 은닉 중계망에 동원된 장비는 `relay_node`다.
- 방어기관이 통제하는 sinkhole은 `sinkhole`이며 relay 또는 C2로 승격하지 않는다.
- 역할이 불명확하면 `unknown`으로 두고 추정하지 않는다.
