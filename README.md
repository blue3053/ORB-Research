...
> 2026-07-16 상태: Phase E(Stage 6–7) prospective scheduling, per-record observed-time
> gate, canonical candidate ledger, independent evidence와 append-only grade history 구현 및
> 오프라인 회귀 검증을 완료했습니다. 상세 결과는
> [Phase E 완료 보고서](docs/2026-07-16-phase-e-completion.md)를 기준으로 합니다.

> 2026-07-16 상태: Phase D(Stage 5) query composition·review·immutable freeze 구현과
> 오프라인 회귀 검증을 완료했습니다. 상세 결과는
> [Phase D 완료 보고서](docs/2026-07-16-phase-d-completion.md)를 기준으로 합니다.

> 2026-07-16 상태: Phase C(Stage 4) feature/background 구현과 오프라인 회귀 검증을 완료했습니다.
> 상세 결과는 [Phase C 완료 보고서](docs/2026-07-16-phase-c-completion.md)를 기준으로 합니다.
...
다음 구현 단계는 Phase F Stage 8 RQ 분석과 재현 패키지입니다.
...
현재 코드는 설계도의 **Phase A–E 완료 기준선**을 제공합니다. Stage 8 분석은 아직 구현되지 않았습니다.
...
| Stage 2 | 완료 | Q0 landmark, append-only timeline, continuity 판정·검토 |
| Stage 3 | 완료 | bounded Q1 precheck, partial 차단, 사람 검토 기반 Q2 진입 |
| Stage 4 | 완료 | feature catalog, entity epoch, matched background, cutoff별 통계·검토 |
| Stage 5 | 완료 | provenance-gated composer, review, budget/schedule, immutable freeze manifest |
| Stage 6 | 완료 | due/missed/late/partial 상태, per-record prospective time, canonical candidate event |
| Stage 7 | 완료 | discovery/validation 분리, 독립 evidence, human adjudication, grade history |
| Stage 8 | 미구현 | RQ1–RQ5 analyzer와 재현 패키지 |

가장 이른 미통과 게이트는 Stage 8 RQ analyzer와 재현 패키지입니다.
...
cti-audit-phase-b
feature-build
feature-build-background
feature-assess
feature-review
cti-audit-phase-c
query-compose
query-register-schedule
query-record-precheck
query-review
query-freeze-designed
cti-audit-phase-d
phase-e-schedule
...
Phase E prospective/candidate 명령은 구현되었습니다. Stage 8의 `analyze`와
`audit leakage` 계열은 다음 Phase F 구현 범위입니다.
...
다음 구현 단위는 Phase F Stage 8의 RQ analyzer와 재현 패키지입니다.

- RQ1–RQ5 입력 eligibility와 분석 cohort 동결
- candidate/adjudication/grade history 기반 전향 성능 집계
- unresolved·unobservable·partial을 분모에서 임의 제거하지 않는 결측 보고
- leakage audit, source-family 민감도 분석, cutoff 재검증
- 분석 결과·표·그림을 manifest/hash와 연결한 재현 패키지
